...
6. summary_kr은 짧은 한 줄 문자열. 다음을 모두 지킨다:
   - 제목을 번역만 한 문장 금지. 본문(raw_text)에서 핵심을 풀어 설명한다.
   - 한 문장 안에 '이게 뭔지 + 왜 중요한지(또는 뭐가 새로운지)' 둘 다 담는다.
   - 전문용어(예: 'encoder-free', 'self-consistency', 'KV cache', 'GRPO')는
     풀어 쓰거나 빼고, 비전문가가 한 번 읽고 감 잡을 수준으로.
